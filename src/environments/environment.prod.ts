import type { AppEnvironment } from './environment.model';

const runtime = (window as any).__env ?? {};
const asBoolean = (value: unknown, fallback: boolean): boolean =>
  value === undefined ? fallback : String(value).toLowerCase() === 'true';

// =========================================================
// Environment – PRODUCCIÓN
// =========================================================
export const environment = {
  production: true,

  /** URL base del API REST (IAM). Cambia según el entorno. */
  API_BASE_URL:
    runtime.API_BASE_URL || 'https://apidev.atu.gob.pe/api-subsidio/api_iam',

  /** URL base del API de Padrón (carga masiva). */
  API_PADRON_URL:
    runtime.API_PADRON_URL || 'https://apidev.atu.gob.pe/api-subsidio/api_padron',

  /** URL base del API de Organización (entidades y ámbitos). */
  API_ORG_URL:
    runtime.API_ORG_URL || 'https://apidev.atu.gob.pe/api-subsidio/api_org',

  /** URL base del API de Validación. */
  API_VALIDACION_URL:
    runtime.API_VALIDACION_URL || 'https://apidev.atu.gob.pe/api-subsidio/api_validacion',

  /** URL base del API de Comprobantes. */
  API_COMPROBANTE_URL:
    runtime.API_COMPROBANTE_URL || 'https://apidev.atu.gob.pe/api-subsidio/api_comprobante',

  /** En producción usa el backend, salvo que se habilite explícitamente el mock. */
  API_COMPROBANTE_MOCK: asBoolean(runtime.API_COMPROBANTE_MOCK, false),

  RECAPTCHA_SITE_KEY:
    runtime.RECAPTCHA_SITE_KEY || '6Ldu6FUrAAAAADnOURKYc9E_uUbGBRC35_ntvznt',

  /** Clave privada para descifrado de campos sensibles (IAM) */
  ENCRYPTION_PRIVATE_KEY:
    runtime.ENCRYPTION_PRIVATE_KEY ||
    '',
  appId: runtime.appId || 'APP_COMPROBANTE',
} satisfies AppEnvironment;
